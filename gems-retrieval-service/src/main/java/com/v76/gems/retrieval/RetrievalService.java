...
public class RetrievalService {
    ...
    public RetrieveResponse retrieve(RetrieveRequest request) {
        ...
        log.info("Cache miss - retrieving embeddings and database matches for query: '{}' with topK: {}", request.query(), topK);
        float[] embedding = embeddingClient.embed(request.query());
        java.util.List<SearchResult> results = repository.search(embedding, topK);
        log.debug("Database vector similarity search returned {} matches for query: '{}'", results != null ? results.size() : 0, request.query());
