...
public class ChatService {
    ...
    public ChatService(@NonNull RetrievalClient retrievalClient, @NonNull PromptBuilder promptBuilder, @NonNull TextGenerationClient textGenerationClient, ObjectMapper objectMapper) {
    ...
    @Trace
    public reactor.core.publisher.Flux<org.springframework.http.codec.ServerSentEvent<String>> streamChat(@NonNull ChatRequest request) {
        Objects.requireNonNull(request);
        long searchStart = System.currentTimeMillis();
        RetrieveResponse retrieveResponse = retrievalClient.retrieve(request.question());
        long searchDuration = System.currentTimeMillis() - searchStart;
        ...
        reactor.core.publisher.Flux<org.springframework.http.codec.ServerSentEvent<String>> sourcesEvent = 
        ...
        reactor.core.publisher.Flux<org.springframework.http.codec.ServerSentEvent<String>> contentEvents = 
                textGenerationClient.stream(prompt)
                        .map(chatResponse -> {
                            String token = chatResponse.getResult() != null ? chatResponse.getResult().getOutput().getText() : "";
        ...
        reactor.core.publisher.Flux<org.springframework.http.codec.ServerSentEvent<String>> doneEvent = 
                reactor.core.publisher.Flux.just(
                        org.springframework.http.codec.ServerSentEvent.<String>builder()
                                .event("done")
                                .data("{}")
